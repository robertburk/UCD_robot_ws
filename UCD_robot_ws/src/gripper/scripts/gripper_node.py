...
FORCE_ACCURACY = 0.05 # Accepted force accuracy: +/- 0.05N
POSITION_ACCURACY = 2
...
class GripperV1():
    ...
    #     if (self.init_move_enabled):
    #         if ( self.mode == 'error'):
    #             self.target_delta = OPEN_LIMIT
    #             rospy.loginfo('[gripper] error mode target: %d' % self.target_delta)

    #         elif ( self.mode == 'limit'):
    #             if np.abs(self.measured_fZ - self.target_force) > FORCE_ACCURACY:
    #                 # self.target_delta = self.pid_limit(self.measured_fZ)
    #                 rospy.loginfo('[gripper] pid limit mode target: %d' % self.target_delta)

    #         elif ( self.mode == 'explore' ):         
    #             self.target_delta = CLOSE_LIMIT
    #             if ( self.prev_target_position != CLOSE_LIMIT ):
    #                 rospy.loginfo('[gripper] explore mode target: %d' % self.target_delta)

    #         elif ( self.mode == 'pid' ) :
    #             if np.abs(self.measured_fZ - self.target_force) > FORCE_ACCURACY:
    #                 self.target_delta = self.pid(self.measured_fZ)
    #                 rospy.loginfo('[gripper] pid mode target: %d' % self.target_delta)
    ...
    # def set_target_position(self):
    #     if (self.init_move_enabled):
    #         if ( self.target_delta == CLOSE_LIMIT ) and ( self.prev_target_position != CLOSE_LIMIT):
    #             self.target_position = CLOSE_LIMIT

    #         elif (self.target_delta != CLOSE_LIMIT) and ( np.abs(self.target_delta) > 2):
    #             read_success = False
    #             while ( not read_success ):
    #                 position = self.controller.MX28_pos()
    #                 if ( type(position) != None ):
